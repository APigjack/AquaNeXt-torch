...
import glob
import rasterio
from new.weights.AquaNeXt.AquaNeXt import AquaNeXt
from new.QD.AquaNeXt_D2.AquaNeXt_D2 import AquaNeXt_D2
from new.QD.AquaNeXt_NH.AquaNeXt_NH import AquaNeXt_NH
from new.QD.AquaNeXt_tech.AquaNeXt_tech import AquaNeXt_tech
from new.QD.AquaNeXt_ST.AquaNeXt_ST import AquaNeXt_ST
...
def build_model(model_type):
    if model_type == "AquaNeXt":
        ...
        is_multi_task = True
    elif model_type == "AquaNeXt_D2":
        model = AquaNeXt_D2(num_classes=2, use_ms_cam_edge=False, use_ms_cam_shared=False)
        is_multi_task = True
    elif model_type == "AquaNeXt_NH":
        model = AquaNeXt_NH(num_classes=2, use_ms_cam=False)
        is_multi_task = True
    elif model_type == "AquaNeXt_tech":
        model = AquaNeXt_tech(num_classes=2, use_ms_cam=False)
        is_multi_task = True
    elif model_type == "AquaNeXt_ST":
        model = AquaNeXt_ST(num_classes=2, use_ms_cam=False)
        is_multi_task = False
    else:
...
def main():
    # 设置参数
    image_path = r'C:\Users\Administrator\Desktop\paper_results\gd_trans\img.tif'
    label_path = r'C:\Users\Administrator\Desktop\paper_results\gd_trans\lbl.tif'
    model_file = r'D:\Paper_code_pt\new\Trans\gd\AquaNeXt_NM_gd_transfromqd\best_val_model_epoch_165.pt'
    output_folder = r'C:\Users\Administrator\Desktop\paper_results\gd_trans\384_0.15_2'
    tile_size = 384
    overlap = 0.15
    model_type = 'AquaNeXt'
    predict_edge = True
